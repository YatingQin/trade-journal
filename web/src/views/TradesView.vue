    ...
    <div class="panel">
      <div class="grid-2">
        ...
        <div class="field">
          ...
          <select id="status" v-model="filters.status" @change="handleLoad">
        ...
        <div class="field">
          ...
          <select id="strategy" v-model="filters.strategy_id" @change="handleLoad">
            <option value="">全部</option>
            <option
        ...
        >
          <div class="trade-item__main">
            <div class="trade-item__title">
              ...
              <span
                class="tag"
                :class="trade.status === 'open' ? 'tag-open' : (trade.is_win ? 'tag-win' : 'tag-loss')"
              >
                {{ trade.status === 'open' ? '持仓中' : (trade.is_win ? '盈利' : '亏损') }}
              </span>
            </div>
            <p>
              {{ trade.account_name }} ·
              {{ trade.strategy_name || '未关联策略' }}
            </p>
            <p>
            ...
            <p v-if="trade.status === 'closed'">
              盈亏
              <strong :class="trade.pnl >= 0 ? 'value-win' : 'value-loss'">
                {{ formatMoney(trade.pnl, trade.account_currency) }}
              </strong>
              ·
              <BaseReasonTag
                v-if="trade.exit_reason_name"
                :reason-type="trade.exit_reason_type || 'other'"
                :label="trade.exit_reason_name"
              />
              <span v-else class="tag">未填原因</span>
            </p>
          </div>
...
async function handleLoad() {
  ...
  try {
    ...
    const [tradeRes, strategyRes] = await Promise.all([
      apiGetTrades(params),
      apiGetStrategies()
    ])
    ...
    errorMessage.value = error.response?.data?.message || '交易列表加载失败'
  }
}
