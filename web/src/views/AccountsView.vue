    ...
    <form class="panel" @submit.prevent="handleSubmit">
      ...
      <div class="grid-2">
        ...
        <div class="field">
          ...
          <select id="market" v-model="form.market" required>
      ...
      <div class="field">
        ...
        <input id="note" v-model.trim="form.note" />
      </div>
    ...
    <div class="panel">
      ...
      >
        <div>
          <h3>{{ item.name }}</h3>
          <p>
            {{ formatMarket(item.market) }} · {{ item.currency }}
...
async function handleSubmit() {
  ...
  try {
    ...
    if (form.id) {
      await apiUpdateAccount(form.id, payload)
    } else {
