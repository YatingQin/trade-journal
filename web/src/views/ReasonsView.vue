...
<template>
  ...
  <section>
    ...
    <form class="panel" @submit.prevent="handleSubmit">
      ...
      <div class="grid-2">
        <div class="field">
          <label for="name">原因名称</label>
          <input id="name" v-model.trim="form.name" required />
        </div>
        <div class="field">
          <label for="reason_type">类型</label>
          <select id="reason_type" v-model="form.reason_type" required>
    ...
    <div class="panel">
      ...
      >
        <div>
          <h3 class="reason-name-row">
            <span>{{ item.name }}</span>
            <BaseReasonTag :reason-type="item.reason_type" />
          </h3>
        </div>
        <div class="inline-actions">
          <button class="btn btn-secondary" type="button" @click="handleEdit(item)">
            编辑
          </button>
          <button class="btn btn-danger" type="button" @click="handleDelete(item.id)">
            删除
          </button>
        </div>
      </div>
...
async function handleLoad() {
  errorMessage.value = ''
  try {
    const { data } = await apiGetExitReasons()
    list.value = data
  } catch (error) {
...
function handleEdit(item) {
  form.id = item.id
  form.name = item.name
  form.reason_type = item.reason_type
}
...
async function handleSubmit() {
  ...
  try {
    ...
    if (form.id) {
      await apiUpdateExitReason(form.id, payload)
    } else {
      await apiCreateExitReason(payload)
    }
    handleReset()
    await handleLoad()
  } catch (error) {
...
async function handleDelete(id) {
  if (!window.confirm('确认删除该原因？')) {
    return
  }
  try {
    await apiDeleteExitReason(id)
    if (form.id === id) {
      handleReset()
    }
    await handleLoad()
